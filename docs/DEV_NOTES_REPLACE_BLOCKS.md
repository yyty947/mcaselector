...
- Builder block and biome autocomplete popups also constrain their horizontal position and width to the owning Builder window. They keep the editor's left edge when possible, shrink to the available right-side space before shifting left, and retain the existing vertical late-resize anchor workaround.
