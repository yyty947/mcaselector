...
- Phase 4G: built-in presets are visible input fillers with air/container warnings; custom presets use selected-rule/draft/all-rules save precedence and append on load. Phase 6 release hardening is implemented. The 2026-07-25 Builder dropdown/first-popup rerun covered the preceding candidate; the later bidirectional popup-attachment change still requires one focused packaged-candidate rerun before PR submission.
