...
- Builder block and biome autocomplete popups also constrain their horizontal position and width to the owning Builder window. They keep the editor's left edge when possible, shrink to the available right-side space before shifting left, and attach their content edge to the field in JavaFX's native upward or downward direction after initial and late geometry changes.
...
- JavaFX 21 can leave an auto-fixed ComboBox popup at a stale Y position after its first post-show resize (`JDK-8338145`). While Builder block and biome autocomplete popups are visible, they track the visible `ListView` height plus the popup window's height/Y changes, choose direction from the popup/control centers, and anchor the list's actual layout edge to the corresponding field edge after initial or late peer geometry updates; listeners are removed when the popup hides. Ordinary property, Extra NBT, and preset dropdown behavior is unchanged.
...
- All Builder ComboBoxes add `replace-blocks-builder-dropdown` to their popup `ListView`. That scoped class removes the project-wide near-black ComboBox border so the popup retains JavaFX's native two-layer frame and shadow like the main menu; do not replace it with a copied fixed gray or change the global ComboBox border as part of Builder polish.
...
- Clicking empty space in the rule table clears its selection so the documented all-rules preset fallback remains reachable.
- Builder editing is transactional: the initially restored rules and draft controls form a clean content snapshot. Dirty state compares the current rule order, raw From/To drafts, source restrictions, and property choices with that snapshot; popup, validation, selection, preset-selection, and catalogue-only state do not count. This means an unchanged restored rule closes silently, reverting a draft to its opening value becomes clean again, and a confirmed catalogue reset stays dirty when it removed opening content.
- JavaFX 21 heavyweight dialogs forward the title-bar close through the underlying `Stage`. Consuming only `DialogEvent.DIALOG_CLOSE_REQUEST` does not consume that original `WindowEvent`, so the Builder installs a lifecycle-scoped `WINDOW_CLOSE_REQUEST` filter and routes Builder Cancel through the same discard decision. The Dialog close handler remains as the Esc/programmatic-close fallback, with one-shot approval preventing duplicate prompts.
- Declining discard, or closing the discard confirmation itself, keeps the Builder open. Explicit discard returns no Builder result and therefore preserves the outer Change NBT ReplaceBlocks text; only Builder OK applies a valid non-empty generated value.
...
- Phase 4G: built-in presets are visible input fillers with air/container warnings; custom presets use selected-rule/draft/all-rules save precedence and append on load. Phase 6 release hardening is complete. The exact final packaged candidate passed the focused Builder popup attachment, keyboard, repeated empty-Biome, native-frame, catalogue/reset, and transactional-close rerun by user report on 2026-07-28.
