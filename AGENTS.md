...
- Phase 6 automated hardening is implemented: 58 tests pass, including Builder rule/preset restoration, pre-1.18 fail-closed execution, modern preview/execution parity, relight flags, and heightmap packing/writeback. Translation completeness and `build shadowJar` pass.
- Packaging, DataVersion 2860/4671 file checks, real biome boundaries, Minecraft load/save/reload, and game-log review have evidence. The remaining release gates are a focused dual-locale Builder rerun, a 1.21 in-game relight check after `isLightOn=0`, final screenshots, and a final JDK FX package rerun. Do not mark Phase 6 complete until they pass.
