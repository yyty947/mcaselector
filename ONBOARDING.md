...
- UI polish: ReplaceBlocks field-row validation waits for a short typing pause, the default NBT Changer width shows the Builder button, empty Builder From/To inputs start blank, Builder validation stays quiet until user action creates a real diagnostic, and Builder From/To inputs have auto-opening A-Z filtered suggestions with blue match highlights, Tab/click completion, boundary-only keyboard scrolling, explicit empty-arrow full-catalog expansion, stronger Builder-only dropdown highlights, Builder-local Preview beside Help, and a Help dialog that contains the advanced-input guidance and Builder-specific explanations. The Builder toolbar is compact, the catalogue selector shows a short Java version with the full DataVersion in its tooltip, source-only conditions share a full-width restrictions strip, empty rules/results use compact localized placeholders, populated areas expand naturally, the Add Rule action keeps the shared button style, generated values wrap in a monospace result area, and block/biome autocomplete popups stay within the Builder window horizontally while preserving their vertical anchor.
- Builder performance hardening bounds synchronous popup-position feedback, limits JavaFX's long-list popup width scan, uses plain text cells for explicit empty-query catalogues, and preloads the immutable block-state catalogue on a daemon thread when Change NBT opens. Candidate data, filtering results, generated text, parser behavior, and execution semantics are unchanged.
- Startup installs a process-wide uncaught-exception handler before version initialization. Uncaught Java exceptions are written at FATAL level to the normal Log4j file and are also printed to standard error; this does not capture a native JVM crash or a thread hang.
...
- On a low-end physical Dell Inspiron 5498 running Java 21, the previous build took about one second for the first From/To expansion and 1-3 seconds for the first `a` filter update. Java Flight Recorder samples concentrated in JavaFX popup width measurement, CSS, and full-list updates.
...
- Automated compile, the complete test suite, `build`, and `shadowJar` pass. The final low-end-device keyboard, popup, style, preset, rule-editing, and advanced-text regression passed by user report on 2026-07-25.
...
- Advanced text and block-state SNBT guidance lives in the Builder Help dialog rather than occupying a permanent footer below the generated value.
...
- Builder editing is transactional. Existing ReplaceBlocks text is the clean opening baseline; closing it unchanged is silent. After rules or draft content change, Builder Cancel, Esc, and the title-bar close button ask whether to discard. `Continue editing` and the confirmation window's close button keep the Builder open; `Discard changes` closes it while preserving the outer Change NBT ReplaceBlocks text. A confirmed catalogue reset remains dirty when it removed opening content.
...
- When investigating a JavaFX failure, inspect both `mcaselector.log` and timestamped `mcaselector-*.log` files because the current log is rolled on each startup. A zero-byte current file does not prove that the preceding run produced no log.
